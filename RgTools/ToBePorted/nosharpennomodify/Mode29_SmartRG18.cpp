#ifdef	SHARPEN
void	SmartRG18(BYTE *dp, int dpitch, const BYTE *_sp, int spitch, int hblocks, int remainder, int incpitch, int height, int strength)
#else
void	SmartRG18(BYTE *dp, int dpitch, const BYTE *_sp, int spitch, int hblocks, int remainder, int incpitch, int height)
#endif
{
		...
		do
		{
			...
			do
			{
				...
				int max1 = min(_sp[0], _sp[2 * spitch  + 1]);
				Max = max(Max, max1);
				int min1 = max(_sp[0], _sp[2 * spitch  + 1]);
				Min = min(Min, min1);
				max1 = min(_sp[0], _sp[spitch  + 2]);
				Max = max(Max, max1);
				min1 = max(_sp[0], _sp[spitch  + 2]);
				Min = min(Min, min1);
				max1 = min(_sp[1], _sp[2*spitch  + 1]);
				Max = max(Max, max1);
				min1 = max(_sp[1], _sp[2*spitch  + 1]);
				Min = min(Min, min1);
				max1 = min(_sp[1], _sp[2*spitch]);
				Max = max(Max, max1);
				min1 = max(_sp[1], _sp[2*spitch]);
				Min = min(Min, min1);
				max1 = min(_sp[1], _sp[2*spitch  + 2]);
				Max = max(Max, max1);
				min1 = max(_sp[1], _sp[2*spitch  + 2]);
				Min = min(Min, min1);
				max1 = min(_sp[2], _sp[2*spitch]);
				Max = max(Max, max1);
				min1 = max(_sp[2], _sp[2*spitch]);
				Min = min(Min, min1);
				max1 = min(_sp[2], _sp[spitch]);
				Max = max(Max, max1);
				min1 = max(_sp[2], _sp[spitch]);
				Min = min(Min, min1);
				max1 = min(_sp[2], _sp[2*spitch + 1]);
				Max = max(Max, max1);
				min1 = max(_sp[2], _sp[2*spitch + 1]);
				Min = min(Min, min1);
				max1 = min(_sp[spitch], _sp[spitch + 2]);
				Max = max(Max, max1);
				min1 = max(_sp[spitch], _sp[spitch + 2]);
				Min = min(Min, min1);
				max1 = min(_sp[spitch], _sp[2*spitch + 2]);
				Max = max(Max, max1);
				min1 = max(_sp[spitch], _sp[2*spitch + 2]);
				Min = min(Min, min1);
				max1 = min(_sp[spitch + 2], _sp[2*spitch]);
				Max = max(Max, max1);
				min1 = max(_sp[spitch + 2], _sp[2*spitch]);
				Min = min(Min, min1);
				if( Min > Max )
				{
					max1 = Max;
...
__asm	SSE_RMOVE	SSE5,				SSE6
__asm	SSE3_MOVE	SSE0,				[esi + ebx + 2]
__asm	pminub		SSE5,				SSE7
__asm	SSE_RMOVE	SSE1,				SSE0
__asm	pmaxub		SSE6,				SSE7
__asm	pminub		SSE0,				SSE5
__asm	SSE3_MOVE	SSE2,				[esi]
__asm	pmaxub		SSE1,				SSE6
__asm	SSE3_MOVE	SSE4,				[esi + 2*ebx]
__asm	pmaxub		SSE0,				SSE2
__asm	pminub		SSE5,				SSE4
__asm	SSE3_MOVE	SSE3,				[esi + 1]
__asm	pminub		SSE1,				SSE2	// SSE2 can be reused
__asm	pmaxub		SSE6,				SSE4
__asm	pmaxub		SSE5,				SSE3
__asm	SSE3_MOVE	SSE2,				[esi + ebx]
__asm	pminub		SSE6,				SSE3	// SSE3 can be reused
__asm	pminub		SSE0,				SSE5	// SSE5 can be reused
__asm	SSE_RMOVE	SSE3,				SSE7
__asm	pmaxub		SSE1,				SSE6	// SSE6 can be reused
__asm	movd		[edi],				SSE2	// copy pixel at [esi + ebx]
__asm	pmaxub		SSE3,				SSE2
__asm	SSE_RMOVE	SSE5,				SSE4
__asm	pminub		SSE7,				SSE2
__asm	SSE3_MOVE	SSE6,				[esi + ebx + 2]
__asm	pminub		SSE0,				SSE3	// SSE3 can be reused
__asm	pminub		SSE4,				SSE2	
__asm	pmaxub		SSE1,				SSE7	// SSE7 can be reused
__asm	SSE_RMOVE	SSE3,				SSE6
__asm	SSE3_MOVE	SSE7,				[esi + 2*ebx + 1]
__asm	pmaxub		SSE5,				SSE2	// SSE2 can be reused
__asm	pmaxub		SSE3,				SSE4
__asm	pminub		SSE6,				SSE5
__asm	pminub		SSE0,				SSE3	// SSE3 can be reused
__asm	SSE3_MOVE	SSE2,				[esi + 2]
__asm	pmaxub		SSE1,				SSE6	// SSE6 can be reused
__asm	pminub		SSE4,				SSE7	
__asm	pmaxub		SSE5,				SSE7	// SSE7 can be reused
__asm	pmaxub		SSE4,				SSE2	
__asm	pminub		SSE5,				SSE2	// SSE2 can be reused
__asm	pminub		SSE0,				SSE4	// SSE4 can be reused
__asm	pmaxub		SSE1,				SSE5	// SSE5 can be reused
